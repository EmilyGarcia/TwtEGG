...
class APIManager: SessionManager {
    ...
    static let consumerKey = "S7NjeEt2OZyTESQJJd2louME7"
    static let consumerSecret = "4VrHZR0B2Xyben33WWxZp09d4FIq28s5q0LiLpLKO0Z1JFogxa"
...
//         tweets,
    ...
    func retweet(_ tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
        let idString = tweet.idString
        let urlString = "https://api.twitter.com/1.1/statuses/retweet/" + idString + ".json"
        let parameters = ["id": tweet.id]
        request(urlString, method: .post, parameters: parameters, encoding: URLEncoding.queryString).validate().responseJSON { (response) in
            if response.result.isSuccess,
                let tweetDictionary = response.result.value as? [String: Any] {
    ...
    func unretweet(_ tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
        let idString = tweet.idString
        let urlString = "https://api.twitter.com/1.1/statuses/unretweet/" + idString + ".json"
        let parameters = ["id": tweet.id]
        request(urlString, method: .post, parameters: parameters, encoding: URLEncoding.queryString).validate().responseJSON { (response) in
            if response.result.isSuccess,
                let tweetDictionary = response.result.value as? [String: Any] {
    ...
    func getMentionsTimeLine(completion: @escaping ([Tweet]?, Error?) -> ()) {
        ...
        //         This uses tweets from disk to avoid hitting rate limit. Comment out if you want fresh
        //         tweets,
        if let data = UserDefaults.standard.object(forKey: "mentions_tweets") as? Data {
            let tweetDictionaries = NSKeyedUnarchiver.unarchiveObject(with: data) as! [[String: Any]]
            let tweets = tweetDictionaries.flatMap({ (dictionary) -> Tweet in
                Tweet(dictionary: dictionary)
            })
